...
(defvar simpc-mode-syntax-table
  (let ((table (make-syntax-table)))
    ...
    (modify-syntax-entry ?& "." table)
    (modify-syntax-entry ?% "." table)
    ;; Backslash is an escape (needed for macro line continuations)
    (modify-syntax-entry ?\\ "\\" table)
    table))
...
(defvar simpc-font-lock-keywords
  (list
   ...
   (cons (regexp-opt (simpc-keywords) 'symbols) 'font-lock-keyword-face)
   (cons (regexp-opt (simpc-types) 'symbols) 'font-lock-type-face))
